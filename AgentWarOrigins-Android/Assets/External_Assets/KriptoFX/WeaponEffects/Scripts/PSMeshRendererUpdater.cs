...
public class PSMeshRendererUpdater : MonoBehaviour
{
    ...
    public void UpdateMeshEffect(GameObject go,SkinnedMeshRenderer mainOutfitSkinnedMeshRenderer)
    {
        rendererMaterials.Clear();
        skinnedMaterials.Clear();
        if (go == null)
        ...
        MeshObject = go;
        UpdatePSMesh(MeshObject,mainOutfitSkinnedMeshRenderer);
        AddMaterialToMesh(MeshObject);
    }

    private void UpdatePSMesh(GameObject go, SkinnedMeshRenderer mainOutift)
    {
        var ps = GetComponentsInChildren<ParticleSystem>();
        var meshRend = go.GetComponentInChildren<MeshRenderer>();
        SkinnedMeshRenderer skinMeshRend;
        if (mainOutift)
        {
            skinMeshRend = mainOutift;
        }
        else
        {
            skinMeshRend = go.GetComponentInChildren<SkinnedMeshRenderer>();
        }
    ...
    Material[] AddToSharedMaterial(Material[] sharedMaterials, WFX_MeshMaterialEffect meshMatEffect)
    {
        ...
        {
            List<Material> sharedMat = new List<Material> {meshMatEffect.Material};

            foreach (var material in sharedMaterials)
                sharedMat.Add(material);
            return sharedMat.ToArray();
        }
    ...
    public void RemoveEffect(GameObject go)
    {
        ...
        foreach (var skinMeshRenderer in skinMeshRenderers)
        {
            skinMeshRenderer.sharedMaterials = RemoveSharedMaterial(skinMeshRenderer.sharedMaterials);
        }
    }

    public Material[] RemoveSharedMaterial(Material[] sharedMaterials)
    {
        List<Material> newList = sharedMaterials.ToList();
        newList.RemoveAt(0);
