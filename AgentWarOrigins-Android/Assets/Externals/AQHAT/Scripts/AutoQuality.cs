...
public class AutoQuality : MonoBehaviour {
    ...
    public static AutoQuality Instance;
	[Tooltip("")]
	...
	public void runAll() {
		...
		if (Application.platform != RuntimePlatform.tvOS && Application.platform != RuntimePlatform.WebGLPlayer) {
#elif UNITY_5
	...
	public void writeQuality(int qualSet) {
#if UNITY_5_3_OR_NEWER || UNITY_5_3
		if (Application.platform != RuntimePlatform.tvOS && Application.platform != RuntimePlatform.WebGLPlayer && Application.platform != RuntimePlatform.SamsungTVPlayer) {
#elif UNITY_5
		if (Application.platform != RuntimePlatform.WebGLPlayer && Application.platform != RuntimePlatform.SamsungTVPlayer) {
    ...
    public int detectQuality(bool usecache,bool applyQuality)
    {

        if (cacheContents != "" && usecache)
        {
            int currentQuality = parseInt(cacheContents);
            if (applyQuality)
            {
    ...
    public int getCacheContents()
    {
        return parseInt(cacheContents);
    }
}
