...
namespace EndlessRunner
{
    ...
    public class ScoreObjectCountRule
    {
        ...
        public bool CanSpawnObject(float distance, ObjectType thisObjectType, int targetObjectIndex, ObjectType targetObjectType)
        {
            ...
            if (thisObjectType == ObjectType.Scene)
            {
                distance = infiniteObjectHistory.GetTotalSceneDistance();
            }
            if (minDistanceSameObjectType)
            {
               ...
               // Debug.Log("last spawn distance of " + thisObjectType +"is" + lastSpawnDistance);
                if (distance - lastSpawnDistance < minDistance)
               {
                        return false;
                }
            ...
            if (distance - objectLastDistance <= minDistance)
                return false;
        ...
        public float ProbabilityAdjustment(float distance, int targetObjectIndex, ObjectType targetObjectType)
        {
            ...
            float totalDistance = infiniteObjectHistory.GetTotalSceneDistance();
            // objectLastDistance: distance of the last spawned object of the same type
            float objectLastDistance = infiniteObjectHistory.GetSpawnDistance(targetObjectIndex);
    ...
    public class ObjectRuleMap
    {
        ...
        public bool ProbabilityAdjustment(InfiniteObjectHistory infiniteObjectHistory, float distance, ref float localDistance, ref float probability)
        {
            ...
            for (int i = 0; i < rules.Count; ++i)
            {
                ...
                {
                    localDistance = infiniteObjectHistory.GetTotalSceneDistance() - infiniteObjectHistory.GetSpawnDistance(targetObjectIndex);
                    return true;
                }
            }
            return false;
     ...
     */
    [System.Serializable]
    public class ScenePlacementRule
    {
        public InfiniteObject scene;
        public bool canSpawn;

        public int sceneIndex;

        public ScenePlacementRule(InfiniteObject p, bool c)
        {
            scene = p;
            canSpawn = c;
        }
