...
public class PrefabLightmapData : MonoBehaviour
{
    ...
    [SerializeField] private bool m_EnableShadowMask;
    
    [SerializeField]
  public  static string LIGHTMAP_RESOURCE_PATH = "Assets/Resources/Lightmaps/NewCity1/";
    [System.Serializable]
    ...
    void Awake()
    {
        ...
        {
            ApplyLightmaps(m_RendererInfo, m_Lightmaps, m_Lightmaps2, m_ShadowMasks);
        }
        else
        {
    ...
    static void ApplyLightmaps(List<RendererInfo> rendererInfo, Texture2D[] lightmaps, Texture2D[] lightmaps2,Texture2D[] shadowMasks)
    {
        bool existsAlready = false;
        int counter = 0;
        int[] lightmapArrayOffsetIndex;
        ...
        for (int i = 0; i < lightmaps.Length; i++)
        {
            ...
            if (!existsAlready)
            {
                ...
                newLightmapData.lightmapColor = lightmaps[i];
                newLightmapData.lightmapDir = lightmaps2[i];
                newLightmapData.shadowMask = shadowMasks[i];
                combinedLightmaps.Add(newLightmapData);
                ++counter;
            }
        }
  ...
  public  static void GenerateLightmapInfo(string folderPath,TextureImporterFormat textureFormat,int textureMaxSize)
    {
        ...
        foreach (var instance in prefabs)
        {
            ...
            if (light.bakingOutput.mixedLightingMode == MixedLightingMode.Shadowmask)
            {
            ...
            {
                GenerateLightmapInfo(scenePath, resourcePath, gameObject, rendererInfos, lightmaps, lightmaps2, shadowMasks);
            }
            ...
            //}
            if (instance.m_EnableShadowMask)
            {
                ApplyLightmaps(instance.m_RendererInfo, instance.m_Lightmaps, instance.m_Lightmaps2, instance.m_ShadowMasks);
    ...
    static void GenerateLightmapInfo(string scenePath, string resourcePath, GameObject root, List<RendererInfo> rendererInfos, List<Texture2D> lightmaps, List<Texture2D> lightmaps2,List<Texture2D> shadowMasks)
    {
        ...
        foreach (MeshRenderer renderer in renderers)
        {
            if (renderer.lightmapIndex != -1 && renderer.enabled)
            {
                ...
                int sceneLightmapIndex = AddLightmap(scenePath, resourcePath, renderer.lightmapIndex, lightmap, lightmap2,shadowMask);
                ...
                if (info.lightmapIndex == -1)
                {
                    ...
                    shadowMasks.Add(sceneLightmaps[sceneLightmapIndex].shadowMask);
                }
    ...
    static int AddLightmap(string scenePath, string resourcePath, int originalLightmapIndex, Texture2D lightmap, Texture2D lightmap2,Texture2D shadowMask)
    {
        int newIndex = -1;
        ...
        if (newIndex == -1)
        {
            ...
            if (lightmap2 != null)
            {
                Debug.Log("lightmap 2 not null");
                lightmap_Remap.lightmap2 = GetLightmapAsset(filename + "_comp_dir.png", resourcePath + "_dir", originalLightmapIndex, lightmap2);
            }

            if (shadowMask != null)
            {
                lightmap_Remap.shadowMask = GetLightmapAsset(filename + "_comp_shadowmask.png",
                    resourcePath + "_shadowMask", originalLightmapIndex, shadowMask);
            }
    ...
    private static Texture2D GetLightmapAsset(string filename, string resourcePath, int originalLightmapIndex, Texture2D lightmap)
    {
